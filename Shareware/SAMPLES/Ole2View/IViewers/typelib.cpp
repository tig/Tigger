...
//
// ITypeLib and ITypeInfo interface viewers.
//
...
IMPLEMENT_OLECREATE(CTypeLibViewer, "Ole2View TypeLib Interface Viewers", 0x57efbf49, 0x4a8b, 0x11ce, 0x87, 0xb, 0x8, 0x0, 0x36, 0x8d, 0x23, 0x2)
...
HRESULT CTypeLibViewer::OnView(HWND hwndParent, REFIID riid, LPUNKNOWN punk)
{
    ...
    else if (punk && riid == IID_ITypeInfo)
    {
...
{
    // both m_pti & m_ptlb cannot be non-null at the same time.
    ASSERT(m_pti != NULL || m_ptlb != NULL) ;
