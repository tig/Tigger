...
IMPLEMENT_DYNCREATE(CTypeLibODLView, CRichEditView)
...
BEGIN_MESSAGE_MAP(CTypeLibODLView, CRichEditView)
...
HRESULT CTypeLibODLView::DeCompileTypeLib( IStream* pstm, ITypeLib* ptlb, UINT uiIndent /* = 0 */ )
{
    ...
    try
    {
		...
		szGUID[37] = '\0' ;
        WRITE1(_T("  uuid(")) ; WRITE(szGUID+1); WRITECR(_T("),"))  ;
        ...
        //{
        //}
        WRITELINE(_T("// NOTE:  There most likely were \"importlib()\" statements in"));
        WRITELINE(_T("//        in the source.  This version of Ole2View does not support"));
        WRITELINE(_T("//        identifying and displaying them."));
        WRITELINE(_T("// "));
        ...
        for (UINT n = 0 ; n < uiInfos ; n++)
        {
            ...
            case TKIND_DISPATCH:
                ...
                {
                    // Get at the interface part.
                    ITypeInfo* ptiInterface = NULL ;
                    HREFTYPE hreftype = NULL ;

                    if (FAILED(hr = pti->GetRefTypeOfImplType((UINT)-1, &hreftype )))
                        AfxThrowOleException(hr) ;
                    // ptiInterface is the interface version of a dual interface.
                    if (FAILED(hr = pti->GetRefTypeInfo(hreftype, &ptiInterface)))
                        AfxThrowOleException(hr) ;
                    hr = DeCompileInterface( pstm, ptiInterface, uiIndent + 1 ) ;
                    ptiInterface->Release();
                }
                else
...
HRESULT CTypeLibODLView::DeCompileModule( IStream* pstm, ITypeInfo* pti, UINT uiIndent /* = 0 */ )
{
    ...
    try
    {
		...
		{
			WRITELINE(_T("// NOTE: This module has no entry points and thus is invalid."));
			WRITELINE(_T("//       There is no way to extract the dllname of a module"));
			WRITELINE(_T("//       with no entry points"));
			WRITELINE(_T("// "));
			hr = S_OK ;
		}
		WRITELINE(_T("[")) ;
...
HRESULT CTypeLibODLView::DeCompileInterface( IStream* pstm, ITypeInfo* pti, UINT uiIndent /* = 0 */ )
{
    ...
    try
    {
        ...
        if (pattr->wTypeFlags & TYPEFLAG_FCANCREATE)
        {
            WRITECR(",") ;
            WRITE1("  cancreate") ;
        }
        ...
        if (pattr->wTypeFlags & TYPEFLAG_FPREDECLID)
        {
            WRITECR(",") ;
            WRITE1("  predeclid") ;
        }
        ...
        int    implflags = NULL ;
        if (SUCCEEDED(pti->GetImplTypeFlags(0, &implflags )))
        {
...
/*
        if (pattr->typekind == TKIND_DISPATCH && pattr->wTypeFlags & TYPEFLAG_FDUAL)
            iStart = 7 ; // skip IDispatch methods
*/

        for (n = iStart ; n < pattr->cFuncs ; n++)
            DumpFunc( pstm, pti, pattr, n, uiIndent + 1 ) ;
...
HRESULT CTypeLibODLView::DeCompileCoClass( IStream* pstm, ITypeInfo* pti, UINT uiIndent /* = 0 */ )
{
    ...
    try
    {
        ...
        for (UINT n = 0 ; n < pattr->cImplTypes ; n++)
        {
            ...
            int impltype = NULL ;
            if (SUCCEEDED(pti->GetImplTypeFlags( n, &impltype )))
            {
                if (FAILED(hr = pti->GetRefTypeOfImplType(n, &href)))
                    AfxThrowOleException(hr) ;
...
HRESULT CTypeLibODLView::DumpFunc( IStream* pstm, ITypeInfo* pti, TYPEATTR* pattr, MEMBERID memid, UINT uiIndent /* = 0 */ )
{
    ...
    try
    {
        ...
        //      helpcontext(<id>)
        //
        // If (pattr->typekind == TKIND_INTERFACE || TKIND_MODULE) || wTypeFlags & TYPEFLAGS_FDUAL
        //  [attributes] returntype [calling convention]  funcname(params);
        // where attributes can be
        ...
        if (pattr->typekind == TKIND_DISPATCH || pattr->wTypeFlags & TYPEFLAG_FDUAL)
        {
            fAttributes = TRUE ;
            fAttribute = TRUE ;
            str.Format(_T("[id(%#08.8x)"), pfuncdesc->memid) ;
            WRITE(str) ;
        }
        else if (pattr->typekind == TKIND_MODULE)
        {
            fAttributes = TRUE ;
            fAttribute = TRUE ;
            str.Format(_T("[entry(%#08.8x)"), pfuncdesc->memid) ;
            WRITE(str) ;
        }
        else
        ...
        if (pattr->typekind == TKIND_DISPATCH && pattr->wTypeFlags & TYPEFLAG_FDUAL)
        {
            // Dual interface methods always return HRESULT
            WRITE("HRESULT") ;
        }
        else
        */
        ...
        /*
        // For dual interfaces, the last param is the return value.
        //
        if ((pattr->typekind == TKIND_DISPATCH) && 
            (pattr->wTypeFlags & TYPEFLAG_FDUAL) && 
...
HRESULT CTypeLibODLView::DumpVar( IStream* pstm, ITypeInfo* pti, TYPEATTR* pattr, MEMBERID memid, UINT uiIndent /* = 0 */)
{
    ...
    try
    {
        ...
        if (pattr->typekind == TKIND_DISPATCH)
        {
            CString str ;
            fAttributes = TRUE ;
            str.Format(_T("[id(%#08.8x)"), pvardesc->memid) ;
            WRITE(str) ;
    ...
    try
    {
        ...
        if (FAILED(hr = VariantChangeType( &varValue, pvardesc->lpvarValue, 0, VT_BSTR )))
        {
            if (pvardesc->lpvarValue->vt == VT_ERROR || pvardesc->lpvarValue->vt == VT_HRESULT)
            {
                varValue.vt = VT_BSTR ;
                TCHAR szSrc[256];
                lstrcpy( szSrc, _GetScodeString(pvardesc->lpvarValue->scode) );
                varValue.bstrVal = SysAllocString(T2OLE(szSrc)) ;
                hr = S_OK ;
            }
            else
                AfxThrowOleException( hr ) ;
        ...
        if (fConst)
        {
            ...
            {
                str.Format(_T("[entry(%#08.8x)"), memid) ;
                WRITE1(str) ;
                fIndent = TRUE ;
            }
