...
LRESULT CALLBACK fnMainWnd( HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam )
{
   switch( wMsg )
   {
      case WM_CREATE:
         ...
         /*
          * Create client window.  We do all the painting
          * in this guy.
          */
         if (!(hwndClient = CreateWindow(
                                       SZ_CLIENTCLASSNAME,
                                       "",
                                       WS_CHILD |
                                       WS_CLIPSIBLINGS |
                                       ...
                                       1,
                                       hWnd,
                                       (HMENU)IDCHILD_CLIENT,
                                       hinstApp,
                                       NULL )))
         {
            DP1( hWDB, "Client create failed!" ) ;
            return -1 ;
         }
...
/****************************************************************
 *  LRESULT CALLBACK fnClientWnd( HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam )
 *
 *  Description: 
 *
 *
...
LRESULT CALLBACK fnClientWnd( HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam )
{
   switch( wMsg )
   {
      case WM_PAINT:
      {
         ...
         y += tm.tmHeight + tm.tmInternalLeading + (CY_DEFAULT) ;
...
VOID WINAPI DrawSample( HDC hDC, LPRECT lprc, LPSTR lpszSample )
{
   ...
   for (lps = lps2 = lpszSample, n = 0 ; *lps2 ; lps2++)
   {
      ...
      if (size.cx > dx)
      {
         GetTextExtentABCPoint( hDC, lps, n+1, &size ) ;
   ...
   if (*lps)
   {
      GetTextExtentABCPoint( hDC, lps, n, &size ) ;
