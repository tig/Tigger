...
// Revision history
//
//  June 28, 1993   First implementation for MSDN Article
//  May, 1994		Picked up by CDK Runtime.  Tested & Debugged.
//	July 27, 1995	Updated to reflect final Cairo properties spec.
...
BOOL CPropertySection::WriteToStream( IStream* pIStream )
{
	...
	Set(PROPID_DICTIONARY, NULL, VT_EMPTY);
	
	// If the code page has not been specified, create ID == 1, and
	// specify Unicode.
	if ((pProp = GetProperty(PROPID_CODEPAGE)) == NULL)
	{
		// The Unicode codepage is 1200.  Property ID 1 is of type
		// VT_I2
		ULONG ul = 1200 ;
		Set(PROPID_CODEPAGE, &ul, VT_I2) ;
	}
	...
	// Now write each property to pIStream.
	POSITION pos = m_PropList.GetHeadPosition();
	while( pos != NULL )
	{
		...
		if (pProp->m_dwPropID != PROPID_DICTIONARY)
		{
			// Write it
	...
	LARGE_INTEGER liPropSet )
{
	...
	// Find the codepage property if it exists.  We need to read it first,
	// because the format of any string properties (including the
	// dictionary) are dependent on codepage (ANSI/DBCS v. Unicode)
	//
	...
	while (cProperties--)
	{
		...
		{
			pIStrPIDO->Release();
			TRACE0("Property set appears corrupt.\n");
			return FALSE;
		}
		
		if (pido.propertyID == PROPID_CODEPAGE)
		{
	...
	// Now move the seek pointer back to just after the section header.
	//
	LISet32(li, ulSectionStart.LowPart + sizeof(m_SH)) ;
	pIStream->Seek(li, STREAM_SEEK_SET, NULL);
	
	pIStream->Clone(&pIStrPIDO) ;
	
	// If the writer didn't include a codepage indicator, we have to assume
	// ANSI/DBCS.  Just use the current ACP.
	if ((pProp = GetProperty(PROPID_CODEPAGE)) == NULL)
	{
		// GetACP returns the current ANSI codepage for the system.  
		// Property ID 1 is of type
		// VT_I2
		ULONG ul = (ULONG)GetACP() ;
		Set(PROPID_CODEPAGE, &ul, VT_I2) ;
	}
	
	// Now actually read the properties, skipping the code page
	// since we arleady read it.
	cProperties = m_SH.cProperties;
	while (cProperties--)
	{
		pIStrPIDO->Read( (LPVOID)&pido, sizeof( pido ), &cb );
		...
		if (pido.propertyID != PROPID_DICTIONARY && pido.propertyID != PROPID_CODEPAGE)
		{
			pProp = new CProperty( pido.propertyID, NULL, 0 );
			pProp->ReadFromStream( pIStream );
			pProp->AssertValid();
		...
		else if (pido.propertyID == PROPID_DICTIONARY)
		{
...
BOOL CPropertySection::SetName( DWORD dwPropID, LPCTSTR pszName )
{
	...
	// BUGBUG: Why is this storing in all lowercase?
...
// Added 7/28/95 
BOOL CPropertySection::GetName( DWORD dwPropID, CString& strName )
{
	DWORD dw ;
	CString str ;
	for( POSITION pos = m_NameDict.GetStartPosition(); pos != NULL;)
	{
		m_NameDict.GetNextAssoc( pos, strName, (void*&)dw  );
		if (dw == dwPropID)
			return TRUE ;
	}
	strName = "" ;
...
struct DICTENTRY
{
	...
	// BUGBUG:  Should be WCHAR and 128 chars.
...
BOOL CPropertySection::ReadNameDictFromStream( IStream* pIStream )
{
	...
	for (iProp = 0; iProp < cProperties; iProp++)
	{
		...
		// BUGBUG:  Persistent form is ANSI/DBCS if the code page is not
		// Unicode.  It is Unicode otherwise.
...
CPropertySet::CPropertySet( void )
{
	...
	m_PH.cSections = 0; // Must never be > 1
...
CPropertySection* CPropertySet::AddSection( CLSID FormatID )
{
	...
	// Support for multiple sections has been removed from the specification.
	// Cairo will only see the first section.
	//
	if (m_PH.cSections > 0)
		return NULL ;
...
void CPropertySet::AddSection( CPropertySection* pSect )
{
	...
	if (m_PH.cSections > 0)
	{
		TRACE("Attempt to add more than one section!\n");
		ASSERT(0) ;
		return ;
	}
...
BOOL CPropertySet::WriteToStream( IStream* pIStream )
{
	...
	// BUGBUG: This could should not even attempt to loop.  There
	// can only be one section!
	if (m_PH.cSections > 1)
	{
		TRACE0("Warning: Writing property set with more than one section.\n");
	}
...
BOOL CPropertySet::ReadFromStream( IStream* pIStream )
{
	...
	if (m_PH.cSections > 1)
	{
		TRACE0("Warning: Reading property set with more than one section.\n");
	}
...
//+--------------------------------------------------------------------------
// Function:    NtGuidToPropertySetName
//
