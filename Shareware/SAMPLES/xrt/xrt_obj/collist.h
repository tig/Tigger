...
//
typedef struct _far tagCOLINFO
{
    UINT        uiWidth ;
...
public:
    ...
    BOOL GetColumns( UINT nCols, UINT FAR* rgColInfo ) ;

    BOOL SetColumnsEx( UINT nCols, COLINFO FAR* rgColInfo ) ;
    BOOL GetColumnsEx( UINT nCols, COLINFO FAR* rgColInfo ) ;
...
protected:
    void JustifiedTextOut( CDC *pDC, int x, int y, 
                           RECT &rc, LPSTR sz, 
                           int nChars = 0, UINT uiJust = DT_LEFT ) ;
    UINT GetHeaderRect( LPRECT lprc, BOOL fScreen = FALSE ) ;
    void DrawBtnFrame( CDC* pDC, LPRECT lprc, BOOL fDepressed = FALSE ) ;
