...
//
//  This code has not been thouroughly tested.  I am confident
//  that everything works as advertised, but I may have
//  missed someting.  If you find a bug in this code
//  please do not hesitate to contact me so that I
//  can fix it and make an updated version available.
//                                                             
// NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE NOTE 
//
...
LRESULT CColumnListBox::OnGetTabStops( WPARAM nTabStops, LPARAM lpnTabStops ) 
{
    if (m_rgTabStops != NULL)
        delete m_rgTabStops ;
        
    m_rgTabStops = new _far int[m_cColumns] ;   // will be freed in destructor
    ...
    nTabStops = (WPARAM)m_cColumns ;
    lpnTabStops = (LPARAM)m_rgTabStops ;            
...
LRESULT CColumnListBox::OnGetColumns( WPARAM nCols, LPARAM rgColWidth ) 
{    return OnGetTabStops( nCols, rgColWidth ) ;  }
...
LRESULT CColumnListBox::OnGetColumnsEx( WPARAM nCols, LPARAM rgColInfo ) 
{
    rgColInfo = (LPARAM)m_rgColInfo ;
    nCols = (WPARAM)m_cColumns ;
...
// FALSE otherwise.
//
// HACK:   This is not implemented yet.  
//
...
{             
    ...
    COLINFO FAR* rgtemp = new _far COLINFO[m_cTabsInHeaderString+1] ;
        ...
        if (PeekMessage((LPMSG)&msg, NULL, WM_MOUSEFIRST, WM_MOUSELAST, PM_REMOVE))
        {
            switch (msg.message)
            {
                case WM_LBUTTONUP:
                    ...
                    if (GetStyle() & LBS_USETABSTOPS)
                    {
                        int far* lpn = new _far int[m_cColumns] ;
                        int n = 0 ;
                        for (UINT i = 0 ; i < m_cColumns ; i++)
                        {
...
BOOL CColumnListBox::GetColumns( UINT nCols, UINT FAR* rgColInfo ) 
{
    return (BOOL)OnGetColumns( (WPARAM)nCols, (LPARAM)rgColInfo ) ;
}
...
BOOL CColumnListBox::GetColumnsEx( UINT nCols, COLINFO FAR* rgColInfo )
{
    return (BOOL)OnGetColumnsEx( (WPARAM)nCols, (LPARAM)rgColInfo ) ;
}
