   ...
   ReallyCreateFont( HDC hDC, LPSTR lpszFace, LPSTR lpszStyle, UINT nPointSize, UINT uiFlags )
{
   ...
   if (hdcCur)
   {
       ...
       */
      if (lpszStyle)
         lstrcpy( (LPSTR)elf.elfStyle, lpszStyle ) ;
      else
        switch( uiFlags & ~(RCF_NODEFAULT | RCF_STRIKEOUT | RCF_UNDERLINE) )
        {
            case RCF_BOLD:
                lstrcpy( (LPSTR)elf.elfStyle, szBold ) ;
            break ;

            case RCF_ITALIC:
                lstrcpy( (LPSTR)elf.elfStyle, szItalic ) ;
            break ;

            case RCF_BOLD | RCF_ITALIC:
                lstrcpy( (LPSTR)elf.elfStyle, szBold ) ;
                lstrcat( (LPSTR)elf.elfStyle, " " ) ;
                lstrcat( (LPSTR)elf.elfStyle, szItalic ) ;
            break ;

            default:
                lstrcpy( (LPSTR)elf.elfStyle, szRegular ) ;
                lpszStyle = (LPSTR)elf.elfStyle ;
            break ;
        }
                ...
                LPENUMLOGFONT lpELF )
{
   ...
   if (!(nFontType & TRUETYPE_FONTTYPE))
   {
      ...
      if (lpELF->elfStyle)
      {
         ...
         if (!lstrcmpi( (LPSTR)lpELF->elfStyle, szBold ))
         {
            lpELF->elfLogFont.lfWeight = FW_BOLD ;
            return 0 ;
         }

         if (!lstrcmpi( (LPSTR)lpELF->elfStyle, szItalic))
         {
            lpELF->elfLogFont.lfItalic = TRUE ;
            return 0 ;
         }

         if (!lstrcmpi( (LPSTR)lpELF->elfStyle, szBoldItalic ))
         {
            lpELF->elfLogFont.lfWeight = FW_BOLD ;
            lpELF->elfLogFont.lfItalic = TRUE ;
            return 0 ;
    ...
    */
   if (0==lstrcmpi( (LPSTR)lpELF->elfStyle, (LPSTR)lpELFin->elfStyle ))
   {
      *lpELF = *lpELFin;
      return 0;
   }
