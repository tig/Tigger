...
HRESULT InitXRTObject( LPSTR lpszObjName )
{
    ...
    LPUNKNOWN       pUnknown ;
    g_xrtCF_XRTSTOCKS = RegisterClipboardFormat( TEXT("CF_XRTDEMO") ) ;
...
//    return ResultFromScode( S_OK ) ;
    ...
    CLSID   clsid ;
    if (FAILED(hr = CLSIDFromProgID( lpszObjName, &clsid )))
    {
        #ifdef _DEBUG
        TCHAR szBuf[256] ; wsprintf( szBuf, TEXT("CLISDFromProgID( %s ) failed!\r\n"), (LPTSTR)lpszObjName ) ;
    ...
    { 
        ...
        if (SUCCEEDED(hr))         
        {
            ...
            if (g_pAdviseSink != NULL)
            {
                ...
                if (FAILED(hr))
                {
                    AssertSz( 0, TEXT("DAdvise failed") ) ;
                    g_pAdviseSink = NULL ;
                    g_dwAdvConn = 0L ;
                }
            }
        ...
        {   
            AssertSz( 0, TEXT("Could not get g_pDataObject") ) ;
            g_pDataObject = NULL ;
        }   
    ...
    {   
        TCHAR sz[128] ;
        wsprintf( sz, TEXT("CoGetClassObject Failed: %08lX"), (DWORD)hr ) ;
        AssertSz( 0, sz ) ;
    }   
