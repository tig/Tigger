   ...
   ReallyCreateFont( HDC hDC, LPTSTR lpszFace, LPTSTR lpszStyle, UINT nPointSize, UINT uiFlags )
{
   ...
   if (hdcCur)
   {
      ...
      if (lpszFace)
#ifndef WIN32
         EnumFontFamilies( hdcCur, lpszFace, lpfn, (LONG)(LPTSTR)(LPVOID) &elf ) ;
#else
...
LPTSTR TYPEDESCtoString( TYPEDESC FAR* lptdesc )
{
    ...
    if ((lptdesc->vt & 0x0FFF) == VT_PTR)
    {
        ...
        if ((lptdesc->lptdesc->vt & 0x0FFF) == VT_CARRAY)
        {
            ...
            TCHAR szNum[16] ;
            wsprintf( szNum, TEXT("[%u]"), lptdesc->lptdesc->lpadesc->cDims ) ;
            lstrcat( sz, szNum ) ;
        }
    }
    ...
    if ((lptdesc->vt & 0x0FFF) == VT_CARRAY)
    {
        ...
        TCHAR szNum[16] ;
        wsprintf( szNum, TEXT("[%u]"), lptdesc->lpadesc->cDims ) ;
        lstrcat( sz, szNum ) ;
    }
    return sz ;
}
