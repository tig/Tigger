...
static LPSTR rgMenu[][5] =
{
    {" &Real-time",          " ",           " ",
    " WOSA/XRT Real Time AddIn",         " "},
    
    {" &Load Object...",          " XRTCreateObject",           " ",
    " Load an object",         " "},
    
    {" &Free Object...",          " XRTDestroyObject",           " ",
    " Free an object",         " "},
    
    {" &Close",             " XRTExit",      " ",
    " Close the WOSA/XRT AddIn",          " "},
};

#define rgMenuRows (sizeof(rgMenu)/sizeof(rgMenu[0]))
...
extern "C"
int FAR PASCAL LibMain(HANDLE hInstance, UINT wDataSeg,
       UINT wHeapSize, LPSTR lpszCmdLine)
{

#ifdef _DEBUG
    OutputDebugString( TEXT("XRTXLL:  LibMain\r\n") ) ;
#endif
    ...
    for (i=0; i<rgFuncsRows; i++)
    {
        for (j=0; j<7; j++)
        {
            rgFuncs[i][j][0] = (BYTE)lstrlenA(rgFuncs[i][j]+1);
        }
    }
    ...
    for (i=0; i<rgMenuRows; i++)
    {
        for (j=0; j<5; j++)
        {
            rgMenu[i][j][0] = (BYTE) lstrlenA(rgMenu[i][j]+1);
        }
    }
...
//      int                1 (required)
extern "C"
int EXPORT FAR PASCAL WEP (int nArgument)
{
    #ifdef _DEBUG
    OutputDebugString(TEXT("XRTXLL: WEP\r\n")) ;
    #endif
...
#ifdef WIN32
extern "C"
int WINAPI DllMain(HANDLE hInstance, DWORD dw , LPVOID )
{

#ifdef _DEBUG
    OutputDebugString( TEXT("XRTXLL32:  DllMain\r\n") ) ;
#endif

    if (dw == DLL_PROCESS_ATTACH)
        return LibMain( hInstance, 0, 0, NULL ) ;
    else if (dw == DLL_PROCESS_DETACH)
        return WEP ( dw ) ;
    else
        return 0 ;
}
...
//
extern "C"
int EXPORT FAR PASCAL xlAutoOpen(void)
{
    ...
    g_hwndXL = (HWND)xDLL.val.w ;

    Excel4( xlGetInst, &xDLL, 0 ) ;
    g_hinstXL = (HINSTANCE)xDLL.val.w ;
    ...
    {
        AssertSz( 0, TEXT("Could not create DDE Server") ) ;
        return 0 ;
    }
...
//
extern "C"
int EXPORT FAR PASCAL xlAutoClose(void)
{
    int i;
    XLOPER xRes;
    ...
    // exists. If it does, it is deleted using xlfDeleteMenu. 
    //
    ...
    Excel(xlfGetBar, &xRes, 3, TempInt(1), TempStr(" Real-time"), TempInt(0));

    if (xRes.xltype != xltypeErr)
    {
        Excel(xlfDeleteMenu, 0, 2, TempNum(1), TempStr(" Real-time"));
...
//
extern "C"
LPXLOPER EXPORT FAR PASCAL xlAutoRegister(LPXLOPER pxName)
{
...
//
extern "C"
int EXPORT FAR PASCAL xlAutoAdd(void)
{
    // Display a dialog box indicating that the XLL was successfully added //
    Excel(xlcAlert, 0, 2, TempStr(" Real Time Access installed."),
      TempInt(2));
    return 1;
}
...
//
extern "C"
int EXPORT FAR PASCAL xlAutoRemove(void)
{
    // Show a dialog box indicating that the XLL was successfully removed //
    Excel(xlcAlert, 0, 2, TempStr(" Real Time Support removed."),
      TempInt(2));
    return 1;
}
...
//
extern "C"
LPXLOPER EXPORT FAR PASCAL xlAddInManagerInfo(LPXLOPER xAction)
{
    ...
    if(xIntAction.val.w==1)
    {
        ...
        xInfo.val.str = "\026WOSA/XRT Support AddIn";
    }
    else
    {
...
//
extern "C"
int EXPORT FAR PASCAL XRTExit(void)
{
