        ...
        if (pRequest)
        {
            ...
            {   
                ...
                if (pRequest->Match( pStock->szSymbol ))
                {
                    ...
                    UpdateDataItems( pRequest, pStock ) ;
...
BOOL CDataObject::AddRequest( CRequest* pRequest ) 
{
    ...
    UpdateAllItems( NULL, UPDATE_ADDREQUEST, pRequest ) ;
    return TRUE ;
}

BOOL CDataObject::ChangeRequest( CRequest* pItem, const CString& rstrRequest )
{
    if (pItem)
    {
        pItem->m_strRequest = rstrRequest ;
        SetModifiedFlag( TRUE ) ;
        UpdateAllViews( NULL, UPDATE_CHANGEREQUEST, pItem ) ;    
        UpdateAllItems( NULL, UPDATE_CHANGEREQUEST, pItem ) ;
        return TRUE ;
    }
    return FALSE ;
}
    ...
    for( pos = m_lstRequests.GetHeadPosition(); pos != NULL; )
    {
        ...
        if (pItem)
        {
            if (lstrcmpi( pItem->m_strRequest, str ) == 0)
            {
                ...
                UpdateAllItems( NULL, UPDATE_REMOVEREQUEST, NULL ) ;
                return  pItem;
            }
        }
    }
...
CRequest* CDataObject::RemoveRequest( CRequest* pItem ) 
{
    POSITION pos = m_lstRequests.Find( pItem ) ;
    if (pos != NULL)
    {
...
// begin with xrt*.
//
void CDataObject::UpdateDataItems( CRequest* pRequest, PSIMSTOCK pStock )
{
    ...
    // For all the other properties see if we have new data
    //
    for (int n = 1; pos != NULL ;n++)
    {
...
{       
    ...
    for (pos = m_lstDataItems.GetHeadPosition() ;pos != NULL;)
    {
        ...
        // Allocate the update item
        //
        lpUpdateItem = xrtAddUpdateItem( &pXRT, 0,  // BUGBUG:  We don't have the dwID of the Request anywhere!
                          pReqProps->m_dwID, pReqProps->m_lstProps.GetCount() ) ;
...
//
LPDISPATCH CDataObject::Requests(const VARIANT FAR& item)
{
    LPDISPATCH lpDisp = NULL ;
    ...
    {
        TRACE( "new CRequests(this) failed\r\n") ;
    }
    ...
    if (item.vt != VT_ERROR)
    {
        ...
        if (lpDispItem = pItems->GetItem( item ))
        {
            lpDisp->Release() ;
            lpDisp = lpDispItem ;
        }
...
//
LPDISPATCH CDataObject::DataItems(const VARIANT FAR& item)
{
    CDataItems* p = new CDataItems( this ) ;
    ASSERT(p) ;
    
    LPDISPATCH lpDisp = p->GetIDispatch( FALSE );
    
    if (item.vt != VT_ERROR && item.vt != VT_EMPTY)
    {
        lpDisp->Release() ;
        lpDisp = p->GetItem( item ) ;
        ASSERT(lpDisp) ;
    }
...
void CTimerWnd::OnTimer(UINT nIDEvent) 
{
    ...
    if (m_pDataObject->m_pXRTData && m_pDataObject->m_pServerItem != NULL)    
    {
    //TRACE("\r\n***Dump before...\r\n") ;
    //xrtDebugDump( afxDump, m_pDataObject->m_pXRTData ) ;
        m_pDataObject->m_pServerItem->m_fInSendOnDataChange = TRUE ;
        m_pDataObject->m_pServerItem->NotifyChanged() ;
        m_pDataObject->m_pServerItem->m_fInSendOnDataChange = FALSE ;
    //TRACE("***Dump after...\r\n") ;
    //xrtDebugDump( afxDump, m_pDataObject->m_pXRTData ) ;
    }
    
    m_pDataObject->ClearDataItems() ;
