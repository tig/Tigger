...
//
VARIANT CDataItem::GetProperties()
{
    ASSERT(m_pValueProps) ;
    ...
    LPPOINT lpOffset, LPSIZE lpSize)
{
    ...
    TRY
    {
        ...
        {
            pDataSource->CacheData(RegisterClipboardFormat("Link Source"), &stgMedium);
                
            // add CF_LINKSOURCEDESCRIPTOR
            GetObjectDescriptorData(lpOffset, lpSize, &stgMedium);
            pDataSource->CacheData(RegisterClipboardFormat("Link Source Descriptor"), &stgMedium);
        }
    }
    CATCH_ALL(e)
    {
