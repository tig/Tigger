...
BOOL WINAPI DoFileOpen( LPSTR lpszFileName, LPSTR lpszFileTitle ) 
{
   ...
   if (GetOpenFileName(&ofn))
   {
       ...
       */
      if (AddSelectFile( ofn.lpstrFile, &fExisted  ))
      {
         lstrcpy( lpszFileName, ofn.lpstrFile ) ;
         lstrcpy( lpszFileTitle, ofn.lpstrFileTitle ) ;

         /*
          * Pop up a message box asking if the user wants
          * to load this file everytime INIedit starts up.  In
          * other words write it to INIEDIT.INI
          */
         if (!fExisted && IDYES == ErrorResBox( hwndApp, hinstApp,
                                             MB_YESNO |
                                             MB_ICONQUESTION,
                                             IDS_APPTITLE,
                                             IDS_ERR_SAVEOTHERFILE,
                                             (LPSTR)lpszFileName ))
         {
            DP( hWDB, "Saving %s", (LPSTR)lpszFileName ) ;
            SaveOtherFile( lpszFileName ) ;
         }
      ...
      {
         DP1( hWDB, "AddSelectFile returned false!" ) ;
      }
...
BOOL WINAPI DoChooseFont( void )
{
   ...
   GetObject( hfontLB, sizeof( LOGFONT ), (LPSTR)&lf ) ;
