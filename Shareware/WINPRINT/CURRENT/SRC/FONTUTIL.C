...
#include "..\inc\isz.h"
#define szBold       GRCS(IDS_BOLD) 
#define szItalic     GRCS(IDS_ITALIC)
#define szBoldItalic GRCS(IDS_BOLDITALIC)
#define szRegular    GRCS(IDS_REGULAR)
   ...
   GetTextMetricsCorrectly( HDC hDC, LPTEXTMETRIC lpTM )
{
   ...
   else
   {
      ...
      GetTextExtentPoint( hDC, rgchAlphabet, 52, &size ) ;
...
UINT WINAPI GetTextExtentABCPoint( HDC hDC, LPSTR lpsz, int cb, LPSIZE lpSize )
{
   ...
   if (!fWin30 && (lpABC || (lpABC = (LPABC)GlobalAllocPtr( GHND, 256*sizeof(ABC) ))))
   {
      ...
      if (MyGetCharABCWidths( hDC, 0, 255, lpABC ))
      {
         ...
         {
            lpSize->cx += (
                           lpABC[(BYTE)lpsz[n]].abcA + 
                           lpABC[(BYTE)lpsz[n]].abcB +
                           lpABC[(BYTE)lpsz[n]].abcC ) ;
         }

         if( cb )
         {

            A = lpABC[(BYTE)lpsz[0]].abcA;
