 ...
 *   5/15/94   - Updated to support FILETIME which is used in COM and in
 *               Win32.  Now serves as a wrapper around Win32 APIs.
 *
 ************************************************************************/
...
#else
#define hinstApp AfxGetInstanceHandle()
#endif
 ...
 ************************************************************************/
/*************************************************************************
 *  LPSTR FAR PASCAL TimeGetCurDate( LPSTR lpszDate, UINT uiFlags )
 *
 *  Description:
...
// Wrap TimeFormatData such that a FILETIME can be used instead of the tm
// structure.
//
LPSTR WINAPI TimeFormatDateEx( LPSTR lpszDate,
                                 FILETIME FAR* pFileTime,
                                 UINT uiFlags ) 
{
    // Convert FILETIME into the tm structure
    //
    WORD    wTime, wDate ;
    struct tm   tmTime ;
    
    CoFileTimeToDosDateTime( pFileTime, &wDate, &wTime ) ;

    tmTime.tm_year = ((wDate >> 9) & 0x007F) + 10 ;
    tmTime.tm_mon = (wDate >> 5) & 0x000F ;
    tmTime.tm_mday = wDate & 0x001F ;
    tmTime.tm_wday = 0 ;
    tmTime.tm_hour = (wTime >> 11) & 0x001F ;
    tmTime.tm_min = (wTime >> 5) & 0x003F ;
    tmTime.tm_sec = (wTime & 0x001F) * 2 ;

    return TimeFormatDate( lpszDate, &tmTime, uiFlags ) ;
}                                 

/*************************************************************************
 *  LPSTR FAR PASCAL TimeGetCurTime( LPSTR lpszTime, UINT uiFlags )
 *
 *  Description: 
 ...
 *
 *************************************************************************/
LPSTR FAR PASCAL TimeFormatTime( LPSTR lpszTime, struct tm FAR *lpTM, UINT uiFlags )
{
   char szFmt[32] ;
   char szTemp[32] ;
...
// structure.
//
LPSTR WINAPI TimeFormatTimeEx( LPSTR lpszTime,
                                 FILETIME FAR* pFileTime,
                                 UINT uiFlags ) 
{
    // Convert FILETIME into the tm structure
    ...
    return TimeFormatTime( lpszTime, &tmTime, uiFlags ) ;
}                                 
