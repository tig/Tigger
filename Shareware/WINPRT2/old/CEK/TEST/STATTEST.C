...
DWORD FAR PASCAL fnMainWnd( HWND hwnd, UINT wMsg, UINT wParam, LONG lParam )
{
   ...
   switch (wMsg)
   {
      case WM_CREATE:
      {
         ...
         hStatFont = ReallyCreateFontEx( NULL, "Helv", "Regular", 10, RCF_NORMAL ) ;
...
//         SendMessage( hwndTop, ST_SETBORDER, INDEX_GAUGE, (LONG)FALSE ) ;
         ...
         /*
          * set the message color to blue
          */
         SendMessage( hwndBottom, ST_SETCOLOR, (WPARAM)-1, RGB( 0, 0, 255 ) ) ;
        ...
        hwndMuTest = CreateMuScrollTest( hwnd )  ;
      }
      break ;
      ...
      case WM_DRAWITEM:
      {
         ...
         switch( lpdis->itemID )
         {
            case INDEX_GAUGE:
            {
               ...
               wsprintf( szBuf, "%d%%", (UINT)nPercent ) ;
      ...
      case WM_DESTROY:
      {
         ...
         if (IsWindow( hwndMuTest ))
         {
            DestroyWindow( hwndMuTest ) ;
         }
