...
extern HANDLE ghmodDLL;

UINT PASCAL WFormStyleFlags(HWND);
...
HANDLE WINAPI HCtlInfo(void)
    ...
    LoadString(ghmodDLL, IDS_CLASSNAME, pCtlInfo->szClass, CTLCLASS);
    LoadString(ghmodDLL, IDS_FULLNAME,  pCtlInfo->szTitle, CTLTITLE);
    ...
    LoadString(ghmodDLL, IDS_VERTICAL, pCtlInfo->Type[0].szDescr, CTLDESCR);
    ...
    LoadString(ghmodDLL, IDS_HORIZONTAL, pCtlInfo->Type[1].szDescr, CTLDESCR);
 ...
 *  cchMax      UINT maximum length of the string.
 *
 * Return Value:
 *  UINT        Number of characters filled in the buffer.  0 on an error.
 *
 */


UINT WINAPI WTranslateCtlFlags(DWORD dwStyle, LPSTR psz, UINT cchMax)
    {
    UINT        cch=0;
    LPSTR       pszT;
    ...
    if (dwStyle & MSS_HORIZONTAL)
        cch=LoadString(ghmodDLL, IDS_MSS_HORIZONTAL, pszT, cchMax);
    else
        cch=LoadString(ghmodDLL, IDS_MSS_VERTICAL, pszT, cchMax);
    ...
    if (MSS_TEXTHASRANGE & dwStyle)
        {
        cch=LoadString(ghmodDLL, IDS_MSS_TEXTHASRANGE, pszT, cchMax);
        pszT+=cch;
        cchMax-=cch;
        }
    ...
    if (MSS_NOPEGSCROLL & dwStyle)
        {
        cch=LoadString(ghmodDLL, IDS_MSS_NOPEGSCROLL, pszT, cchMax);
        pszT+=cch;
        cchMax-=cch;
        }
    ...
    if (MSS_INVERTRANGE & dwStyle)
        {
        cch=LoadString(ghmodDLL, IDS_MSS_INVERTRANGE, pszT, cchMax);
        pszT+=cch;
        cchMax-=cch;
        }
...
BOOL WINAPI FShowCtlStyleDlg(HWND hWnd, HANDLE hMem,
    ...
    lpStyleDlgProc=MakeProcInstance((FARPROC)FCtlStyleDlgProc, ghmodDLL);

    bReturn=DialogBox(ghmodDLL, MAKEINTRESOURCE(IDD_STYLEDIALOG),
...
BOOL WINAPI FCtlStyleDlgProc(HWND hDlg, UINT uiMsg, WPARAM wParam, LPARAM lParam)
    {
...
/*    PortTool v2.2     10/26/1992    23:43          */
/*      Found   : WM_COMMAND          */
...
BOOL PASCAL FRangePositionCheck(HWND hDlg)
    ...
    if (fTextOK && (iPos < iMin || iPos > iMax))
        {
        LoadString(ghmodDLL, IDS_CLASSNAME,  szTemp1, 60);
        LoadString(ghmodDLL, IDS_RANGEERROR, szTemp2, 60);
