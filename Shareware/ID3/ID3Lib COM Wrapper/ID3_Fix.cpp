...
// Little string compare fn. to remove dep. on crt startup code

inline BOOL scmp5A(const char* sz1, const char* sz2)
{
	if (sz1[0] == sz1[0] &&
		sz1[1] == sz1[1] &&
		sz1[2] == sz1[2] &&
...
{
	char	szFrameID[5];
	if (WideCharToMultiByte(CP_ACP, 0, wszFrameID, 5, szFrameID, 5, 0 ,0) == 0)
		ID3_THROW(ID3E_NoMemory);

	ULONG iFrame = 0;
	while (ID3_FrameDefs[iFrame].id != ID3FID_NOFRAME)
	{
		if (scmp5A(szFrameID, ID3_FrameDefs[iFrame].longTextID))
...
void ID3_Frame_SetStringID(ID3_Frame* pFrame, const char* sz)
{
	...
	ULONG iFrame = 0;
	while (ID3_FrameDefs[iFrame].id != ID3FID_NOFRAME && 
		(!scmp5A(ID3_FrameDefs[iFrame].longTextID, sz)))
		iFrame++;
