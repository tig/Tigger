...
BOOL WINAPI RegisterToolbarClass( HINSTANCE hInstance )
{
    ...
    wc.lpszClassName= SZ_TOOLBARCLASS;
...
LRESULT CALLBACK ToolbarWndProc( HWND hWnd, WORD msg, WPARAM wParam, LPARAM lParam )
{
    ...
    switch (msg)
    {
        case WM_CREATE:
        {
            ...
            if ( !(pTBHdr->hlstBtns = fsmmListCreate( NULL )) )
            {
        ...
        case TBM_ADDBUTTON:
        {
            ...
            if (lpB)
            {
                if (lpB->uiBtnStyle & (BTNS_TEXT|BTNS_TEXTBORDER))
                {
                    ...
                    {
                        HHEAP hhHeap = fsmmListHeap( pTBHdr->hlstBtns );

                        lp = fsmmHeapAllocString( hhHeap, LPTR, ((LPBTNTEXT)lpB)->lpText  );
                        if (!lp)
                            return 0L;
                    }
                }
            ...
            lpNB = (LPBTNINFO)fsmmListAllocInsertNode(pTBHdr->hlstBtns, (LPSTR)lpB,
        ...
        case TBM_REMOVEBUTTON:
        {
            ...
            if (wParam==1)
                ...
                lpB = (LPBTNINFO)fsmmListItem2Node( pTBHdr->hlstBtns, lParam );
            ...
            if (lpB->uiBtnStyle & (BTNS_TEXT|BTNS_TEXTBORDER))
            {
                if ( lpB->dwData )  // Free text
                {
                    ...
                    fsmmHeapUnlockFree( hhHeap, (LPSTR)lpB->dwData );
                }
            }

            fsmmListDeleteNode( pTBHdr->hlstBtns, (LPNODE)lpB );
            InvalidateRect( hWnd, NULL, TRUE );
            return 1L;
        }
        break;
            ...
            if (lParam)
            {
                ...
                if (lpB->uiBtnStyle&BTNS_TOGGLEEX)
                {
                    ...
                    while(1)
                    {
                        ...
                        lpBI2 = (LPBTNINFO)fsmmListPrevNode(pTBHdr->hlstBtns, lpBI);
                    ...
                    while (lpBI && lpBI->uiBtnID)
                    {
                        ...
                        lpBI = (LPBTNINFO)fsmmListNextNode(pTBHdr->hlstBtns, lpBI);
                    }
                    lpB->uiBtnState |= BTN_DOWN;
                }
            }
        ...
        case TBM_SETBTNTEXT:
        {
            ...
            if ( lpB->lpText )  // Free text
                fsmmHeapUnlockFree(hhHeap, lpB->lpText );
            lpB->lpText = NULL;

            if (lParam)
            {
                lpB->lpText = fsmmHeapAllocString(hhHeap, LPTR, (LPSTR)lParam);
                if (!lpB->lpText)
                    return -1L;
            }
...
VOID NEAR CleanUpToolbar( PTBHDR pTBHdr )
{
    ...
    if (pTBHdr->hlstBtns)
        fsmmListDestroy( pTBHdr->hlstBtns );
    LocalFree( (HLOCAL)pTBHdr );
...
LONG NEAR PASCAL HandleMouseButton(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (msg==WM_LBUTTONDOWN)
    {
       ...
       {
         /* Fake a WM_MENUSELECT with this guy selected
          */
         SendMessage( hwndParent, WM_MENUSELECT, lpB->uiBtnID,
            MAKELONG( MF_HILITE, GetMenu( hwndParent ) ) ) ;
       }
...
LONG NEAR PASCAL HandleMouseMove(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (lpB!=pTBHdr->lpB && (pTBHdr->uiState&TBST_LASTHIT) )
    {
       ...
       if ((pTBHdr->lpB->uiBtnStyle & BTNS_MENUID) &&
           (hwndParent = GetParent( hWnd )))
       {
         /* Fake a WM_MENUSELECT with nothing selected...
          */
         SendMessage( hwndParent, WM_MENUSELECT, 0, 0L ) ;
       }
...
void NEAR PASCAL ToolbarPaint( HWND hWnd )
{
    ...
    lpB = (LPBTNINFO)fsmmListLastNode( pTBHdr->hlstBtns );
    ...
    while (lpB)
    {
        ...
        lpB = (LPBTNINFO)fsmmListPrevNode( pTBHdr->hlstBtns, lpB );
    }
...
{
    PTBHDR pTBHdr = (PTBHDR)GetWindowWord( hWnd, WE_PTBHDR );
    LPBTNINFO lpB = (LPBTNINFO)fsmmListFirstNode( pTBHdr->hlstBtns );

    while (lpB)
    {
        if (lpB->uiBtnID == uiID)
            break;
        lpB = (LPBTNINFO)fsmmListNextNode( pTBHdr->hlstBtns, lpB );
    }
    return lpB;
...
VOID NEAR PASCAL GetBtnRect( HWND hWnd, LPBTNINFO lpB, LPRECT lprc)
{
    ...
    lpBI = (LPBTNINFO)fsmmListFirstNode( pTBHdr->hlstBtns );
