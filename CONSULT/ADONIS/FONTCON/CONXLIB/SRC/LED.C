...
#define WNDEXTRABYTES 1*sizeof(UINT)+1*sizeof(LONG)
 ...
 *************************************************************************/
LONG CALLBACK
fnLED( HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam )
{
   switch (wMsg)
   {
      case WM_CREATE:
...
static BOOL NEAR PASCAL Create( HWND hWnd, LPARAM lParam )
{
   ...
   SetWindowWord( hWnd, GWW_FLAGS, (UINT)lpCreate->style & 0x00FF ) ;
