...
//               5/6/92    CEK        Moved to CONXLIB.DLL
//
//*************************************************************
...
LRESULT FAR PASCAL fnSlider( HWND, WORD, WPARAM, LPARAM ) ;
...
BOOL WINAPI SliderInit( HINSTANCE hPrev, HINSTANCE hInstance )
{
   ...
   if (!hPrev)
   {
      ...
      wc.lpszClassName = SZ_SLIDERCLASS;
...
LRESULT FAR PASCAL fnSlider( HWND hWnd, WORD wMsg, WPARAM wParam, LPARAM lParam )
{
   switch (wMsg)
   {
      case WM_CREATE:
      ...
      case WM_MOUSEMOVE:
         return ( MouseMove( hWnd, (WORD) wParam, (LONG) lParam ) ) ;
      ...
      case WM_LBUTTONDOWN:
         return ( MouseLButtonDown( hWnd, (WORD) wParam, (LONG) lParam ) ) ;

      case WM_LBUTTONUP:
         return ( MouseLButtonUp( hWnd, (WORD) wParam, (LONG) lParam ) ) ;

      case SL_SETNUMCOLS:
         return ( SetNumberColumns( hWnd, (WORD) wParam ) ) ;

      case SL_SETCOLHEADER:
         return ( SetColumnHeader( hWnd, (WORD) wParam, (LPSTR) lParam ) ) ;

      case SL_SETCOLALIGN:
         return ( SetColumnAlign( hWnd, (WORD) wParam, LOWORD( lParam ) ) ) ;
      ...
      case SL_SETCOLPOS:
         return ( SetColumnPosition( hWnd, (WORD) wParam,
                                     LOWORD( lParam ) ) ) ;
      ...
      case SL_GETCOLPOS:
         return ( GetColumnPosition( hWnd, (WORD) wParam ) ) ;

      case SL_GETCOLALIGN:
         return ( GetColumnAlign( hWnd, (WORD) wParam ) ) ;
...
HSLIDERINFO NEAR CreateSliderInfo( HWND hWnd, DWORD dwStyle )
{
   ...
   npSliderInfo -> hSplitCursor =
      LoadCursor( hmodDLL, MAKEINTRESOURCE( SPLITCURSOR ) ) ;
...
LRESULT NEAR SetNumberColumns( HWND hWnd, WORD wNumCols )
{
   HSLIDERINFO   hSliderInfo ;
   HCOLUMNINFO   hColumnInfo ;
   NPSLIDERINFO  npSliderInfo ;
...
LRESULT NEAR SetColumnHeader( HWND hWnd, WORD wColumn, LPSTR lpText )
{
   HSLIDERINFO   hSliderInfo ;
   HLOCAL        hColHeader ;
   NPCOLUMNINFO  npColumnInfo ;
   ...
   if ((NULL == npSliderInfo -> hColumnInfo) ||
       (wColumn >= (WORD) npSliderInfo -> nNumCols))
   {
      return ( (LRESULT) FALSE ) ;
   }
...
LRESULT NEAR SetColumnAlign( HWND hWnd, WORD wColumn, WORD wAlignment )
{
   HSLIDERINFO   hSliderInfo ;
   NPCOLUMNINFO  npColumnInfo ;
   NPSLIDERINFO  npSliderInfo ;
...
LRESULT NEAR SetColumnPosition( HWND hWnd, WORD wColumn, WORD wPosition )
{
   HSLIDERINFO   hSliderInfo ;
   NPCOLUMNINFO  npColumnInfo ;
   NPSLIDERINFO  npSliderInfo ;
...
LRESULT NEAR GetColumnPosition( HWND hWnd, WORD wColumn )
{
   HSLIDERINFO   hSliderInfo ;
   NPCOLUMNINFO  npColumnInfo ;
   NPSLIDERINFO  npSliderInfo ;
...
LRESULT NEAR GetColumnAlign( HWND hWnd, WORD wColumn )
{
   HSLIDERINFO   hSliderInfo ;
   NPCOLUMNINFO  npColumnInfo ;
   NPSLIDERINFO  npSliderInfo ;
...
LRESULT NEAR MouseMove( HWND hWnd, WORD wFlags, LONG lPosition )
{
...
LRESULT NEAR MouseLButtonDown( HWND hWnd, WORD wFlags, LONG lPosition )
{
...
LRESULT NEAR MouseLButtonUP( HWND hWnd, WORD wFlags, LONG lPosition )
{
   HSLIDERINFO   hSliderInfo ;
   NPSLIDERINFO  npSliderInfo ;
   NPCOLUMNINFO  npColumnInfo ;
