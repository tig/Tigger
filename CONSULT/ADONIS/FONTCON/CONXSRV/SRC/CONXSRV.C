 ...
 *      Module:  CONXSRV.c
 *
 *    CONXSRV is an asynchronous communications application for
 *    Windows (3.1 and up) whose purpose is to act as a intermediary
 *    between Adonis' shopper applications and the BBS at Adonis.
 *
 *    The shopper applications (clients) communicate with CONXSRV
 *    through DDE (CONXSRV is a DDEML server).  And CONXSRV, in turn,
 *    communicates with the Adonis BBS.
 *
 *    The intent is that CONXSRV could be re-written someday to
 *    communicate with the BBS via a local area network, or some
 *    other means, and no changes would have to be made to the
 *    clients.
 *
...
/*
 * Global variables extern'd in CONXSRV.h
 */
HWND        hwndApp ;
HWND        hwndTTY ;
HWND        hwndStat ;
...
BOOL        fInternal ;    // set if CONXSRV is being used internally and
