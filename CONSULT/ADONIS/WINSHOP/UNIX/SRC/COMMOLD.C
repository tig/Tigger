...
int
set_tty_baud(HANDLE hCom, int baud)
{
...
ReceiveBuf(HANDLE hCom, BYTE *buf, int iBufLen, WORD TimeToWait)
{
    ...
    if (bParity) /* then gotta read one char at a time */
		for (pBuf = buf, nNumLeft = iBufLen; 
			nNumLeft; 
			nNumLeft -= nNumRead, pBuf += nNumRead)
		{
			while ((nNumRead= read(hCom,pBuf,1)) <= 0) 
				if (TimeToWait)
				{
					if (Elapsed_Time(FALSE) > TimeToWait)
					{
						printf("ReceiveBuf:  timed out\n");
						return TIMEOUT_ERR;
					}
				}
				else if (errno)
			...
			#if 0
			if (*pBuf == 0) // then parity error
			{
			    printf("ReceiveBuf:  parity error\n");
				//return PARITY_ERR;
				continue;
			}
			#else
			if (*pBuf == 0377) // then parity error
			{
				char buf[2];
				read(hCom,buf,2);
...
/* receive string matching str or return FAIL. */
{
	...
	while (nNumLeft) 
	{
		while (read(hCom,pBuf,1) <= 0) 
			if (TimeToWait)
			{
				if (Elapsed_Time(FALSE) > TimeToWait)
				{
					printf("ReceiveStr:  timed out\n");
		...
		if (bParity)
		{
			...
			if (*pBuf == 0) // then parity error
			{
			    printf("ReceiveStr:  parity error\n");
			//	return PARITY_ERR;
				continue;
			}
		#else
	...
	// can be NULL
{
	...
	do
	{
		while (read(hCom,&c,1)  <= 0)
			if (TimeToWait)
			{
				if (Elapsed_Time(FALSE) > TimeToWait)
				{
					printf("ReceiveVarBuf:  timed out\n");
					END(TIMEOUT_ERR)
				}
			}
			else if (errno)
