...
LOCALHANDLE hClipIndexMemory;
static int  fClipFileHandle = -1;

VBS     vbs;

static int NEAR PASCAL InitNewLibrary(HWND hWndDlg, BOOL bReInit);
void FAR PASCAL UnloadRCData(LPSTR lpData);
void NEAR PASCAL UnderlineThumbnails(WORD, VBS *, int, int, int, WORD, HWND);
void NEAR PASCAL MoveHighlight(HWND, WORD, LONG, VBS *);

int    NEAR PASCAL SetupClipIndexes(int nNumBitmaps);
int    NEAR PASCAL OpenClipFile(char *szThumbInfoFile);
void   NEAR PASCAL CloseClipFile(int fHandle);
int    NEAR PASCAL GetIndexedClipInfo(int index, char *szClipDesc, char *szClipSize);
...
CreateViewBox (int whichSublib, WORD whichImage)
{
    ...
    else 
    {
        ...
        {
            if ( InitNewLibrary (hWndView, TRUE) == FAIL )  // was FALSE
            {
        ...
        // just change which picture in this sublib
        else
        {
...
WSViewBoxDlgProc (HWND hWndDlg, unsigned message, WORD wParam, LONG lParam)
{
    ...
    switch(message) 
    {
        case WM_INITDIALOG:
            ...
            if (InitNewLibrary(hWndDlg,TRUE) == FAIL)
            {
                SetCursor(hSaveCurs);
        ...
        case WM_CTLCOLOR:
           if(LOWORD(lParam)!=GetDlgItem(hWndDlg, ID_LIST))
               return(FALSE);
#ifdef INTERNAL
           AuxPrint("Processing CTLCOLOR in Viewbox");
#endif
           ...
           for(ii = nCount; ii != (WORD)nTarget; ii += nInc )
           {
                ...
                if (ii == (WORD)nCount) {
                    /*----------------------------------*/
                    /* First time.                      */
                    /* Store the handle to the default  */
                    /* bitmap.                          */
                    /*----------------------------------*/
                ...
                if(ii==vbs.nSelect) {
                    ...
                    if(bCompress) {
                        ...
                        StretchBlt(wParam, ((ii-vbs.nScrollPos)*(rHeight+DELTA_X)+X_START), 
                            yOffset, rHeight, rHeight, hDCMem, 0, 0, WIDTH_BITMAP, HEIGHT_BITMAP, dwROP);
                    }
                    else
                    {
                        BitBlt(wParam, ((ii-vbs.nScrollPos)*(WIDTH_BITMAP+DELTA_X)+X_START), yOffset, 
                            WIDTH_BITMAP, HEIGHT_BITMAP, hDCMem, 0, 0, dwROP);

                    }

                    /*---------------------------*/
                    /* Update all the info stuff */
                    /*---------------------------*/


                    {				// Fix up the ampersands in the string.....
                        char Buffer[MAX_STR_LEN];
                        if (GetIndexedClipInfo(ii, Buffer, CURCLIPSIZE) == FAIL)
                        {
                ...
                ************************************************************/
                ...
                if (ii == vbs.nSelect)
                {
                    ropPrev = SetROP2(wParam, R2_BLACK);
                    cr = GetSysColor(COLOR_HIGHLIGHTTEXT);
                }
                else
                {
                    ropPrev = SetROP2(wParam, R2_WHITE);
                    //ropPrev = SetROP2(wParam, R2_COPYPEN);
                    cr = GetSysColor(COLOR_WINDOW);
                }

                hPen = CreatePen(PS_SOLID, HIGHLIGHT_WIDTH, cr);
                hPenPrev = SelectObject(wParam, hPen);

                xOffset = ((ii-vbs.nScrollPos) * (WIDTH_BITMAP+DELTA_X)+X_START);
                MoveTo(wParam, xOffset-wLine, yOffset-wLine);
                LineTo(wParam, xOffset+WIDTH_BITMAP+wLine, yOffset-wLine);
                LineTo(wParam, xOffset+WIDTH_BITMAP+wLine, yOffset+HEIGHT_BITMAP+wLine);
                LineTo(wParam, xOffset-wLine, yOffset+HEIGHT_BITMAP+wLine);
                LineTo(wParam, xOffset-wLine, yOffset-wLine);

                SetROP2(wParam, ropPrev);
                SelectObject(wParam, hPenPrev);
                DeleteObject(hPen);
                }
                ...
                *
                ***************************************************/
                UnderlineThumbnails(wParam, &vbs, ii, nCount, nTarget-nInc, yOffset+Y_SPACING, hWndDlg);
...
END_DIALOG:
        case WM_CLOSE:
            ...
            if (hClipIndexMemory)
            {
                LocalFree(hClipIndexMemory);
                hClipIndexMemory=NULL;
            }
            CloseClipFile(fClipFileHandle);
            DeleteObject(hBrush);
                    ...
                    switch(HIWORD(lParam)) 
                    {
                        case LBN_SELCHANGE:
                        {
                           ...
                           if (nIndex != (int)wLastSublib)
                           {
                              ...
                              {
                                  if (InitNewLibrary(hWndDlg,FALSE) == FAIL)
                                  {
                                      SendMessage(hWndSubLib, LB_SETCURSEL, wLastSublib, 0L);
                                      /* Kludge to reset the focus in the list box */
                                      InvalidateRect(hWndSubLib, (LPRECT)NULL, TRUE);
...
InitNewLibrary(HWND hWndDlg, BOOL bNewLib)
{
    ...
    vbs.nSelect = imageNumber;
    vbs.nScrollPos = min((int)imageNumber, vbs.nMaxScrollPos);
    ...
    ************************************************/
    CloseClipFile(fClipFileHandle);
    if ((fClipFileHandle = OpenClipFile(CURTHUMBINFO)) == NULL)
    {
        ErrMsg(IDS_ERR_LIB_NOTAVAIL);
        END(FAIL)
    }
...
MoveHighlight(HWND hWndDlg, WORD wParam, LONG lParam, VBS  *vbs)
{
   ...
   /*************************************** KRG ***
   *
   *  Make sure we stay within the boundaries of
   *  the list
   ***********************************************/
   if(vbs->nScrollPos > vbs->nMaxScrollPos)
      vbs->nScrollPos = vbs->nMaxScrollPos;
   else if(vbs->nScrollPos < 0)
      vbs->nScrollPos = 0;

   SetScrollPos(vbs->hWndScroll,
                SB_CTL, vbs->nScrollPos, TRUE);
...
SetupClipIndexes(int nNumBitmaps)
{
    ...
    if(!hClipIndexMemory)
    {
        hClipIndexMemory = LocalAlloc(LHND, sizeof(long)*nNumBitmaps ); // MOVEABLE & Zero Init
        if(!hClipIndexMemory) 
        {
    ...
    {
        hClipIndexMemory = LocalReAlloc(hClipIndexMemory, sizeof(long)*nNumBitmaps , LHND);
        if(!hClipIndexMemory) 
        {
            SetErr(IDS_ERR_LMEM_FULL);
            END(FAIL)
    ...
    plOffset  = (PLONG)LocalLock(hClipIndexMemory);

    while( n < nNumBitmaps && !eof(fClipFileHandle) )
    {
       *plOffset++ = _llseek(fClipFileHandle, 0L, 1);

	    /* read up to next record */
        if (ReadToChar(fClipFileHandle,'') == FAIL)
        {
           SetErr(IDS_ERR_BAD_BITMAP);
           END(FAIL)
        }

        if (ReadToChar(fClipFileHandle,'\n') == FAIL)
        {
           SetErr(IDS_ERR_BAD_BITMAP);
           END(FAIL)
        }
...
OpenClipFile(char *szThumbInfoFile)
{
    ...
    /* open file */
    if ((fHandle = _lopen(szThumbInfoFile,READ)) == -1) 
        return NULL;
    return(fHandle);
}
...
IsClipFree(int index)
{
   ...
   if (NOT CURLIBDLDABLE)
    return FALSE;

   plOffset = (PLONG)LocalLock(hClipIndexMemory);

   _llseek(fClipFileHandle, *(plOffset + index), 0);
...
#ifdef INTERNAL
   AuxPrint("RS from IsClipFree()");
#endif
   if (ReadStringToChar(fClipFileHandle,szTemp,'\t', sizeof(szTemp)) == FAIL)
       return FAIL;

   // Does price string consist of zeros and "." ?, then must be free
   return ((strspn(szTemp, "0.") == strlen(szTemp)));
}

int NEAR PASCAL 
GetIndexedClipInfo(int index, char *szClipDesc, char *szClipSize)
{
   PLONG plOffset;
...
#ifdef INTERNAL
   //AuxPrint("RS from GetIndexedClipInfo(), desc");
#endif
   if (ReadStringToChar(fClipFileHandle, szClipDesc, '\n', MAX_STR_LEN) == FAIL)
      return FAIL;
...
#ifdef INTERNAL
   //AuxPrint("RS from GetIndexedClipInfo(), size");
#endif
   if (ReadStringToChar(fClipFileHandle,szClipSize,'\t', MAX_CLIP_LEN) == FAIL)
       return FAIL;
