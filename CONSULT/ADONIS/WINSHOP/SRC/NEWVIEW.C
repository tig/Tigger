...
static    BOOL    abIsClipFree[7];  // hardwired index
...
VBS     vbs;
typedef fpos_t *POFFSET;  /* to store offsets into thumbinfo file */
...
void   NEAR PASCAL CloseClipFile(FILE* fp);
int    NEAR PASCAL GetClipDesc(int index, char *szClipDesc);
int    NEAR PASCAL IsClipFree(int index);
int    NEAR PASCAL HighlightSelection(int nImage, HDC hDC, BOOL bAddHighLight);
           ...
           for(ii = nCount; ii != (WORD)nTarget; ii += nInc )
           {
                ...
                if (abIsClipFree[abs(ii - nCount)])
                {
                   ...
                   TextOut(wParam,
                           ...
                           yOffset - 20,
                           (LPSTR)"FREE", 4);
                   abIsClipFree[abs(ii-nCount)] = FALSE;
                   SetTextColor(wParam, GetSysColor(COLOR_HIGHLIGHTTEXT));
                }
                ...
                if (abs(ii - nCount) == nPrevSelect)
                    HighlightSelection(ii, wParam, FALSE);

                // Add new highlighting and clip description
                if (ii == vbs.nSelect)
                {
                    char Buffer[MAX_STR_LEN];

                    nPrevSelect = abs(ii - nCount);
                    HighlightSelection(ii, wParam, TRUE);
                ...
                if (CURLIBDLDABLE)
                {
                   switch ( IsClipFree(ii) )
                   {
                      case TRUE :
                      {
                           ...
                           abIsClipFree[abs(ii-nCount)] = TRUE;
                           SetBkColor(wParam, GetSysColor(COLOR_HIGHLIGHT));
                           SetTextColor(wParam, GetSysColor(COLOR_HIGHLIGHTTEXT));
                           break;
                      }
...
SetupClipIndexes(int nNumBitmaps)
{
    ...
    {
        // MOVEABLE & Zero Init
        hClipIndexMemory = LocalAlloc(LHND, sizeof(POFFSET) * nNumBitmaps );
        if(!hClipIndexMemory) 
        {
    ...
    else
    {
        hClipIndexMemory = LocalReAlloc(hClipIndexMemory,
                              sizeof(POFFSET) * nNumBitmaps, LHND);
        if(!hClipIndexMemory) 
        {
            SetErr(IDS_ERR_LMEM_FULL);
            END(FAIL)
    ...
    pOffset  = (POFFSET)LocalLock(hClipIndexMemory);
...
OpenClipFile(char *szThumbInfoFile)
{
    ...
    if ((fpTmp = fdopen(fHandle, "rt")) == NULL)
    {
        ErrMsg(IDS_FILE_IO_ERR,ID_ERR_OPENING_FILE,
                  (LPSTR)StripPath(szThumbInfoFile));
        return ( NULL );
...
int NEAR PASCAL
HighlightSelection(int nImage, HDC hDC, BOOL bAddHighLight)
/************************************************ pt 5/91 ***
...
************************************************************/
{
   ...
   xOffset = ((nImage - vbs.nScrollPos) * (WIDTH_BITMAP + DELTA_X) + X_START);
