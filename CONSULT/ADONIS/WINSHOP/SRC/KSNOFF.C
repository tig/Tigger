...
StartSearch (HWND hWnd)
{
    ...
    else
    {
       ...
       */
       ...
       if ((searchIndexP = fdopen(Search.searchIndex, "rt")) == NULL)
       {
           ErrMsg (IDS_MSG_MISSING_SRCH_FILE);
           return FALSE;
       }
...
SearchPubFile (void)
{
    ...
    // Open search file, and set position
    if ( Search.searchFile == -1 )
    {
       //Get file handle
       ...
       //setup for buffered input
       if ((Search.searchFileP = fdopen(Search.searchFile, "rt")) == NULL)
       {
           ErrMsg (IDS_MSG_MISSING_SRCH_FILE);
           return FALSE;
       }
    ...
    if (NOT Search.searchInProgress)   //reset  static var's
    {
       imageNumber = 0;
       libFile[0] = '\0';
       sublib2File[0] = '\0';
    ...
    while ( !foundOne )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                //chop off last three chars: "description\tPRICE_CODE\n"
                //becomes: "description"
                line[strlen(line) - 3] = '\0';

                //Lines are all uppercase in search files
    ...
    else    // Search index already opened
    {
        ...
        else
        {
            ...
            if ( Search.searchInProgress )
            {
                ...
                SetDlgItemText (hDldWnd, IDS_DESCRIPTION, (LPSTR)szStopText);
                if ( SearchPubFile() )
                {
                    DESTROY_DLDWND
                    return TRUE;
