...
BmpBtnInit( HANDLE hPrev, HANDLE hInst )
{
   ...
   if (!hPrev)
   {
      ...
      if (!RegisterClass( &wc ))
      {
         DP1( hWDB, "BmpBtnInit failed to register the class" );
         return FALSE ;
      }
   }
   else
      DP3( hWDB, "bmpbtnInit, hPrev != NULL" ) ;
...
/*************************************************************************
 *  LONG FAR PASCAL fnBmpBtn( HWND hWnd, WORD wMsg,
 ...
 *************************************************************************/
LONG FAR PASCAL
fnBmpBtn( HWND hWnd, WORD wMsg, WORD wParam, LONG lParam )
{
   HANDLE      hBMup ;
   HANDLE      hBMdn ;
   PAINTSTRUCT ps ;
   ...
   switch (wMsg)
   {
      ...
      break ;
		...
		case WM_LBUTTONDOWN:
			//
			// set focus on a lbutton down!
			//
			if (!IsWindowEnabled( hWnd ))
				return 0L ;

			DP4( hWDB, "WM_LBUTTONDOWN" ) ;
...
DrawGrayButton( HWND hWnd, HDC hDC, LPRECT lpRect, WORD wStyle, BOOL fInvert)
{
   ...
   if (!(wStyle & BS_STATIC))
   {
      ...
      PatB( hDC, Rect.right-iFrame, Rect.top+1,         iFrame, dy-2,
            rgbBlack ) ;
#else
      PatB( hDC, Rect.left,       Rect.top,           dx,   iFrame,
            rgbBlack ) ;

      PatB( hDC, Rect.left,       Rect.bottom-iFrame, dx,   iFrame,
            rgbBlack ) ;
            
      PatB( hDC, Rect.left,         Rect.top,         iFrame, dy,
            rgbBlack ) ;

      PatB( hDC, Rect.right-iFrame, Rect.top,         iFrame, dy,
            rgbBlack ) ;
