                   ...
                   WORD    wFileLen ) 
{
   ...
   DP3( hWDB,  "FileOpenDialog()" ) ;
   if (!(hlFOS = LocalAlloc( LHND, sizeof( FILEOPENSTRUCT ) )))
   {
      DP3( hWDB,  "FileOpenDialog: LocalAlloc failed" ) ;
      return DLG_ERROR ;
   }
   ...
   if (lpfnDlgProc = MakeProcInstance( FileOpenDlgProc, hInst ))
   {
                                   ...
                                   LONG lParam )
{
   ...
   {
      case WM_INITDIALOG:
         DP3( hWDB,  "FileOpenDlgProc: WM_INITDIALOG" ) ;
         ...
         {
            DP3( hWDB,  "FileOpenDlgProc: Could not SetProp" ) ;
            return FALSE ;
         }
      ...
      case WM_COMMAND:
         switch (wParam)
         {
            case IDD_FILES_LB:
               switch (HIWORD( lParam ))
               {
                  ...
                  case LBN_DBLCLK:
                     DlgDirSelect( hDlg, szBuf, IDD_FILES_LB ) ;
                     SetDlgItemText( hDlg, IDD_FILESPEC_EC, szBuf ) ;
                     DP3( hWDB,  "FileSpec = %s", (LPSTR)szBuf ) ;
            ...
            case IDOK:
               ...
               {
                  DP3( hWDB,  "ValidFile: %s", (LPSTR)szBuf ) ;
...
BOOL UpdateDirs( HWND hDlg, LPSTR lpszBuf )
{
   DP3( hWDB,  "Doing a DlgDirList on %s", (LPSTR)lpszBuf ) ;
...
BOOL ValidFile( HWND hDlg, LPSTR lpszFile, LPSTR lpszDefSpec )
{
   ...
   DP3( hWDB, " In ValidFile( %s, %s )", (LPSTR)lpszFile, (LPSTR)lpszDefSpec ) ;

   SplitFileandPath( lpszFile, szPath, szFname ) ;
   DP3( hWDB,  "szPath = %s, szFname = %s", (LPSTR)szPath, (LPSTR)szFname ) ;
