...
int  DlgFnOpen(HWND, unsigned, WORD, LONG);
VOID FAR DlgCheckOkEnable(HWND, int, unsigned);
void DlgAddCorrectExtension(char *, BOOL);
BOOL FAR DlgCheckFilename(char *);
BOOL DlgSearchSpec(char *);
int  FAR DlgSend(HANDLE, HWND, int, OFSTRUCT *, int *, char *, char *, char *);
int  DlgFnSend(HWND, unsigned, WORD, LONG);
...
=============================================================================*/
int DlgFnOpen(hwnd, msg, wParam, lParam)
HWND hwnd;
unsigned msg;
WORD wParam;
LONG lParam;
...
LoadIt:
		...
		/* see if file already exists */
                *pfpSave = OpenFile((LPSTR)szFileNameSave,(LPOFSTRUCT)rgbOpenSave,  OF_READWRITE);
		if (*pfpSave != -1) {
		    EndDialog(hwnd, OLDOPEN);
		    break;
		}

		if (rgbOpenSave->nErrCode == EACCES || rgbOpenSave->nErrCode == EDEADLOCK)
		  {
		  MessageBox(hwnd, "File is locked and can not be accessed", pszAppname,
			     MB_OK | MB_ICONQUESTION | MB_APPLMODAL);
		  }
		else
		  {
		  MessageBox(hwnd, "File does not exist", pszAppname,
			     MB_OK | MB_ICONQUESTION | MB_APPLMODAL);
		  }
		break;
	    }
	...
	case ID_LISTBOX:
		...
		if (DlgDirSelect(hwnd, (LPSTR)szFileNameSave, ID_LISTBOX)) {
		    ...
		    while (pchFile > rgch) {
			pchFile = (char *)AnsiPrev(rgch, pchFile);
                        if (*pchFile == '*' || *pchFile == '?')
			    fWild = TRUE;
                        if (*pchFile == '\\' || *pchFile == ':') {
			    pchFile = (char *)AnsiNext(pchFile);
			    break;
			}
		    }
		    if (fWild)
