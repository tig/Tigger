                                      ...
                                      LPSTR lpFile )
{
   ...
   if (LoadString( hInstLib, IDS_SETTINGS_TRANSLATE, szKey, MAXLEN_TEMPSTR ))
   {
      if (DECGL( npECB ) >= CHARSET_NRCSBRITISH)
         WritePrivateProfileWord( lpSection, szKey,
                                  DECGL( npECB ), lpFile ) ;
   }
   if (LoadString( hInstLib, IDS_SETTINGS_LOCALECHO, szKey, MAXLEN_TEMPSTR ))
      WritePrivateProfileTrueFalse( lpSection, szKey,
                                    LOCALECHO( npECB ), lpFile ) ;
   if (LoadString( hInstLib, IDS_SETTINGS_NEWLINE, szKey, MAXLEN_TEMPSTR ))
      WritePrivateProfileTrueFalse( lpSection, szKey,
                                    NEWLINE( npECB ), lpFile ) ;
   if (LoadString( hInstLib, IDS_SETTINGS_USENUMLOCK, szKey, MAXLEN_TEMPSTR ))
      WritePrivateProfileTrueFalse( lpSection, szKey,
                                    USENUMLOCK( npECB ), lpFile ) ;
//   if (LoadString( hInstLib, IDS_SETTINGS_SENDDEL, szKey, MAXLEN_TEMPSTR ))
//      WritePrivateProfileTrueFalse( lpSection, szKey,
//                                    SENDDEL( npECB ), lpFile ) ;
   if (LoadString( hInstLib, IDS_SETTINGS_LINEWRAP, szKey, MAXLEN_TEMPSTR ))
      WritePrivateProfileTrueFalse( lpSection, szKey,
                                    AUTOWRAP( npECB ), lpFile ) ;
   WriteFonts( lpSection, &NORMALFONT( npECB ),
               &HIGHLIGHTFONT( npECB ), lpFile ) ;
