...
/************************************************************************
 *  BOOL UnregisterClasses( HANDLE hInstance )
 *
 *  Description:
 *
 ...
 *
 *  Comments:
 *      4/17/91  baw  "Wrote this nifty comment block."
                                 ...
                                 WPARAM wParam, LPARAM lParam )
{
   switch (uMsg)
   {
      case WM_CREATE:
      {
         ...
         SetWindowWord( hWnd, GWW_ECB, (WPARAM) hECB ) ;
      }
      break ;
      ...
      case WM_ERASEBKGND:
      {
         ...
         if (NULL == (hECB = GETECB( hWnd )))
            return ( 0 ) ;
         if (NULL == (npECB = (NPECB) LocalLock( hECB )))
            return ( 0 ) ;

         hbrBG = CreateSolidBrush( NORMALFONT.lfBGcolor ) ;
         hbrBG =
