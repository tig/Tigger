...
Spagetti:
            ...
            // If it's a dual interface insert both the interface and dual versions
            //
            // Note this is just about the ugliest piece of code I've ever written.  Sorry.
            //
            if (pattr->typekind == TKIND_DISPATCH && pattr->wTypeFlags & TYPEFLAG_FDUAL)
            {
                pNewItem->GetTypeInfo()->ReleaseTypeAttr(pattr);

                // Get at the interface version
                ITypeInfo* ptiInterface = NULL ;
                HREFTYPE hreftype = NULL ;

                if (FAILED(hr = pNewItem->GetTypeInfo()->GetRefTypeOfImplType((UINT)-1, &hreftype)))
                    AfxThrowOleException(hr) ;
                // ptiInterface is the interface version of a dual interface.
                if (FAILED(hr = pNewItem->GetTypeInfo()->GetRefTypeInfo(hreftype, &ptiInterface)))
                    AfxThrowOleException(hr) ;
...
BOOL CTreeItem::ExpandFuncs( HTREEITEM hitem ) 
{
    ...
    TRY
    {
        ...
        for ( int iIndex = iStart ; iIndex < pattr->cFuncs ; iIndex++)
        {
...
BOOL CTreeItem::ExpandImplTypes( HTREEITEM hitem ) 
{
    ...
    TRY
    {
        ...
        if (pattr->typekind == TKIND_DISPATCH && pattr->wTypeFlags & TYPEFLAG_FDUAL)
        {
            ...
            // Fool the for loop below into only looping once
            pattr->cImplTypes--;
        }

        for (; n <  (int)pattr->cImplTypes; n++)
        {
            pNewItem = new CTreeItem( m_pTree ) ;
            pNewItem->m_uiMemid = (UINT)n ;           
            ASSERT(pNewItem) ;

            tvis.item.lParam = (LPARAM)pNewItem ;
            HREFTYPE href = NULL ;
            hr = pti->GetRefTypeOfImplType((UINT)n, &href) ;
            if (FAILED(hr))
            {
            ...
            pNewItem->m_Type = TypeKindToItemType( pNewItem->GetTypeKind() ) ;
            tvis.item.iImage = TypeKindToItemType( pattrNew->typekind ) ;
            tvis.item.iSelectedImage = tvis.item.iImage ;
