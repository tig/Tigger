 ...
 */
   ...
   #define GALLOC(cb)         GlobalAllocLock( cb )
...
LPVOID WINAPI GlobalAllocLock( DWORD cb );
VOID WINAPI GlobalUnlockFree( LPVOID lp ) ;
...
LRESULT WINAPI fnStatus( HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam )
{
   ...
   switch (wMsg)
   {
      ...
      case ST_GETSTATUSWIDTH:
         return MAKELRESULT( StatusGetWidth( hWnd, (LPSTR)lParam ), 0 ) ;
      break ;
...
/*************************************************************************
 *  WORD WINAPI StatusGetWidth( HWND hWnd, LPSTR lpszText )
 *
 *  Description: 
 *
 *
 ...
 *
 *************************************************************************/
int WINAPI StatusGetWidth( HWND hWnd, LPSTR lpszText )
{
...
static void NEAR PASCAL StatusPaint( HWND hWnd )
{
       ...
       (lpStatus->fTouched || IsRectInRect( &r1, &ps.rcPaint )))
   {
      ...
      if (lstrlen(szText) || !lpStatus->hszDefault)
