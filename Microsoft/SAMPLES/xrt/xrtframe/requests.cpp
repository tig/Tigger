...
{
    if (m_pDataObject) 
        return m_pDataObject->GetRequestCount() ;
    else
        return 0 ;
}
...
LPDISPATCH CRequests::Add(const VARIANT FAR& name, const VARIANT FAR& properties)
{
    ...
    if (pItem != NULL)
    {
        ...
        //
        if (properties.vt == (VT_VARIANT | VT_BYREF))   // it's an object
        {
            if (properties.pvarVal->vt == VT_DISPATCH)
            {
                pProps = (CProperties*)CCmdTarget::FromIDispatch(properties.pvarVal->pdispVal) ;
                if (pProps && pProps->IsKindOf(RUNTIME_CLASS(CProperties)))
