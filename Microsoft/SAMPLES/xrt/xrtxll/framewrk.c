...
#if 0
int far cdecl Excel4(int xlfn, LPXLOPER operRes, int count,... )
{
    return 0 ;
}
...
int cdecl Excel(int xlfn, LPXLOPER pxResult, int count, ...)
{
    ...
    for (i=0; i<count; i++)
    {
        ...
        if (!*ppx)
        {
            debugPrintf(TEXT("Error! A zero parameter is being passed to Excel. Check memory...\r\n"));
            FreeAllTempMemory();
            return xlretFailed;
        }
    ...
    {
        debugPrintf(TEXT("Error! Excel4("));

        if (xlfn & xlCommand)
            debugPrintf(TEXT("xlCommand | "));
        if (xlfn & xlSpecial)
            debugPrintf(TEXT("xlSpecial | "));
        if (xlfn & xlIntl)
            debugPrintf(TEXT("xlIntl | "));
        if (xlfn & xlPrompt)
            debugPrintf(TEXT("xlPrompt | "));

        debugPrintf(TEXT("%u) callback failed:"),xlfn & 0x0FFF);

        /* More than one error bit may be on */

        if (xlret & xlretAbort)
        {
            debugPrintf(TEXT(" Macro Halted\r"));
        }

        if (xlret & xlretInvXlfn)
        {
            debugPrintf(TEXT(" Invalid Function Number\r"));
        }

        if (xlret & xlretInvCount)
        {
            debugPrintf(TEXT(" Invalid Number of Arguments\r"));
        }

        if (xlret & xlretInvXloper)
        {
            debugPrintf(TEXT(" Invalid XLOPER\r"));
        }

        if (xlret & xlretStackOvfl)
        {
            debugPrintf(TEXT(" Stack Overflow\r"));
        }

        if (xlret & xlretFailed)
        {
            debugPrintf(TEXT(" Command failed\r"));
        }

        if (xlret & xlretUncalced)
        {
            debugPrintf(TEXT(" Uncalced cell\r"));
        }
