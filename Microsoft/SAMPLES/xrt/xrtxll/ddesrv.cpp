...
{
    { TEXT("TEXT"),   CF_TEXT,    0, NULL, RequestDDEData },
    { TEXT("Dummy1"),   CF_TEXT,    0, NULL, RequestDDEData },
    { TEXT("Dummy2"),   CF_TEXT,    0, NULL, RequestDDEData }
} ;

DDEITEMTBL DDEItems[] = 
{
    { TEXT("ID"), 0, NULL, sizeof(DDEFormats)/sizeof(DDEFormats[0]), 0, DDEFormats }
} ;

DDETOPICTBL DDETopics[] =
{
    { TEXT("Data"), NULL, sizeof(DDEItems)/sizeof(DDEItems[0]), 0, DDEItems }
} ;

DDESERVICETBL ServiceInfo[] =
{
    { TEXT("XRTXLL"), NULL, sizeof(DDETopics)/sizeof(DDETopics[0]), 0, DDETopics }
} ;
...
{                                 
    ...
    {
        TCHAR sz[128] ;
        wsprintf( sz, TEXT("InitializeDDE Failed!  %#04x"), uiErr ) ;
        AssertSz(0, sz ) ;
    ...
    {         
        #ifdef _DEBUG
        TCHAR sz[128] ;
        wsprintf( sz, TEXT("InitializeDDE Succeeded.  g_idInst == %#08lx\r\n"), (DWORD)g_idInst ) ;
...
BOOL WINAPI KillDDEServer() 
{
    ...
    {
        #ifdef _DEBUG
        OutputDebugString( TEXT("KillDDEServer called when server was never created!\r\n") ) ;
        #endif
        return FALSE ;
    }
        
    #ifdef _DEBUG
    OutputDebugString( TEXT("Uninitializing DDE server....\r\n") ) ;
    #endif
    
    UninitializeDDE( ) ;
    
    #ifdef _DEBUG
    OutputDebugString( TEXT("Uninitialized DDE server.\r\n") ) ;
    #endif
...
void WINAPI PostDDEServer() 
{
    ...
    {
        #ifdef _DEBUG
        OutputDebugString( TEXT("PostDDEServer when g_fServerCreated == FALSE \r\n") ) ;
        #endif
        return ;
    }
                        ...
                        ServiceInfo->topic[0].item[0].hszItem ) == 0)
    {
        #ifdef _DEBUG
        TCHAR sz[128] ;
        wsprintf( sz, TEXT("DdePostAdvise failed:  %#04x\r\n"), DdeGetLastError( g_idInst ) ) ;                        
        OutputDebugString( sz) ;
        #endif
    }
}   
...
{   
    ...
    #ifdef _DEBUG
    wsprintf( sz, TEXT("RequestDDEData %lu\r\n"), g_dwUpdateCount ) ;
    OutputDebugString( sz ) ;
    #endif
               
    wsprintf( sz, TEXT("%lu"), g_dwUpdateCount ) ;
