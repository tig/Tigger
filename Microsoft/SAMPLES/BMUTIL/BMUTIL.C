...
#include <wdb.h>
   ...
   */
   #include "bmutil.h"
#else
   #include "..\inc\colrutil.h"

   /*
   * This header file contains function prototypes and API defines.
   */
   #include "..\inc\bmutil.h"
#endif
...
BOOL NEAR PASCAL IsPixelHigh( COLORREF rgb ) ;
   ...
   if (hbmMask = CreateBitmap( lpBM->bmWidth, lpBM->bmHeight, 1, 1, NULL ))
   {
      if (hdcMask = CreateCompatibleDC( hdcBM ))
      {
         ...
         if (wStyle & BM_DISABLED)
         {
            ...
            COLORREF    rgbNormal = GetSysColor( COLOR_BTNFACE ) ;
            short       x, y ;

            /*
             * For every pixel in the bitmap
             */
            for (y = 0 ; y < lpBM->bmHeight ; y++)
            {
               for (x = 0 ; x < lpBM->bmWidth ; x++)
               {
                  if ((y + 1) == lpBM->bmHeight ||
                      (x + 1) == lpBM->bmWidth)
                  {
                     /*
                      * Special case for the bottom right pixels.
                      *
                      * Just make them "normal"
                      */                       
                     SetPixel( hdcBM, x, y, rgbNormal ) ;
                   ...
                   * Is it "high" or "low"
                   */
                  if (IsPixelHigh( GetPixel( hdcBM, x, y ) ) &&
                      !IsPixelHigh( GetPixel( hdcBM, x+1, y+1) ))
                  {
                     /* Make it "shadow" */
                     SetPixel( hdcBM, x, y, rgbShadow ) ;
                  }
                  else
                     if (!IsPixelHigh( GetPixel( hdcBM, x, y ) ) &&
                        IsPixelHigh( GetPixel( hdcBM, x+1, y+1) ))
                     {
                        /* Make it "highlight */
                        SetPixel( hdcBM, x, y, rgbHighlight ) ;
                     }
                     else
                     {
                        /* Make it "normal" */
 ...
 *  Description: 
 *
 *    Given a RGB value determine if it is high or low (from tinfoil)
 *
 *  Comments:
 *
 ****************************************************************/
BOOL NEAR PASCAL IsPixelHigh( COLORREF rgb )
{
   ...
   if (GetRValue( rgb ) == 255) guns += 1 ;
   if (GetGValue( rgb ) == 255) guns += 1 ;
   if (GetBValue( rgb ) == 255) guns += 1 ;
                                          ...
                                          WORD  wStyle )
{
   ...
   {
      if (HIWORD( lpszResName ))
      {
         DP4( hWDB, "bmLoadBitmapTranslate( %s )", (LPSTR)lpszResName ) ;
      }
   }
   ...
   {
      DP4( hWDB, "bmLoadBitmapTranslate failed" ) ;
   }
...
HBITMAP NEAR PASCAL AutoLoadBitmap( HANDLE hInst, LPSTR lpszRes )
{
   ...
   wsprintf( szName, "%s%dx%dx%d",
            (LPSTR)lpszRes, nLogPelX, nLogPelY, nColors ) ;
   ...
   if (!hBM)
   {
      ...
      wsprintf( szName, "%s%dx%d",
               (LPSTR)lpszRes, nLogPelX, nLogPelY ) ;
   ...
   if (!hBM)
   {
      ...
      //
      DP5( hWDB, "  Trying to load bitmap %s", (LPSTR)lpszRes ) ;
      return LoadBitmap( hInst, lpszRes ) ;
   }
   else
      return hBM ;
