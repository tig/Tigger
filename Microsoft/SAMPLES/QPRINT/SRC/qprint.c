...
/*
 * Global variables extern'd in QPRINT.h
 */
HWND        hwndApp ;
...
LRESULT CALLBACK fnMainWnd( HWND hWnd, UINT uiMsg, WPARAM wParam, LPARAM lParam )
{
   ...
   switch( uiMsg )
   {
      case WM_CREATE:
         ...
         if (!(nTimer = SetTimer( hWnd, 42, 1000, NULL )))
         {
            ErrorResBox( hWnd, hinstApp,
                   MB_ICONHAND, IDS_APPTITLE, IDS_ERR_TIMER ) ;
            return -1 ;
         }
      break ;
      ...
      case WM_TIMER:
      {
         ...
         /* Use _dos_findfirst/next to print and delete all *.TXT
          * files in the root of drive c:
          */

         if (0 == _dos_findfirst( "C:\\*.TXT", _A_NORMAL, &ftFind ))
         {
            do
            {
               wsprintf( szFile, "%s%s", (LPSTR)"C:\\", (LPSTR)ftFind.name ) ;
                                                
               DP2( hWDB, "Printing %s", (LPSTR)szFile ) ;
               PrintFile( szFile ) ;

               DP2( hWDB, "Deleting %s", (LPSTR)szFile ) ;
...
int PASCAL WinMain( HINSTANCE hinst, HINSTANCE hinstPrev, LPSTR lpszCmd, int nCmdShow )
{
   ...
   D( hWDB = wdbOpenSession( NULL, "QPRINT", "wdb.ini", 0 ) ) ;
   ...
   ShowWindow( hwndApp, SW_SHOWMINIMIZED /*nCmdShow*/ ) ;
   UpdateWindow( hwndApp ) ;

   /*
    * Enter message loop.  This is in QPRINT.C so that it is in
    * a preload segment.
    */
   n = MessageLoop(  ) ;
