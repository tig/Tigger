...
LRESULT CALLBACK fnMainWnd( HWND hWnd, UINT uiMsg, WPARAM wParam, LPARAM lParam )
{
   ...
   switch( uiMsg )
   {
      case WM_CREATE:
      {
         ...
         hdc = GetDC( hWnd ) ;
         hfontTest = ReallyCreateFontEx( hdc, "Courier", "Italic", 12, 0 );

        hfontTest = SelectObject( hdc, hfontTest ) ;
         GetTextFace( hdc, 255, szFace ) ;
         GetTextStyle( hdc, 255, szStyle ) ;

         DP1(hWDB, "hdc = %04X, hfontTest = %04X ('%s' '%s' %d point)", hfontTest, hdc,
                (LPSTR)szFace, (LPSTR)szStyle, GetTextPointSize( hdc ) ) ;
        hfontTest = SelectObject( hdc, hfontTest ) ;
      ...
      case WM_COMMAND:
      {
         ...
         switch (wID)
         {
            ...
            case IDM_CHOOSEFONT:
            {
               ...
               chf.hwndOwner = hWnd ;
               chf.lpLogFont = &lf ;
               chf.Flags = CF_SCREENFONTS | /*CF_NOSIMULATIONS | */
               ...
               if (ChooseFont(&chf))
               {
                  ...
                  hfontTest = CreateFontIndirect( &lf ) ;
                    ...
                    DP1(hWDB, "chf.hDC = %04X, hfontTest = %04X ('%s' '%s' %d point)", hfontTest, chf.hDC,
                    (LPSTR)szFace, (LPSTR)szStyle, GetTextPointSize( chf.hDC ) ) ;
                    hfontTest = SelectObject( chf.hDC, hfontTest ) ;
      ...
      case WM_PAINT:
      {
         ...
         hfont = ReallyCreateFontEx( ps.hdc, szFace, szStyle, nPtSize, 0 ) ;

         TextOut( ps.hdc, x, y, szBuf, lstrlen( szBuf ) ) ;

         y += + cyChar ;

         wsprintf( szBuf, "The Full Name for this font is: %s.",
                   (LPSTR)szFullName ) ;
         TextOut( ps.hdc, x, y, szBuf, lstrlen( szBuf ) ) ;
         ...
         wsprintf( szBuf, "Using GetTextExtentPoint (which is stupid):" ) ;
         TextOut( ps.hdc, x, y, szBuf, lstrlen( szBuf ) ) ;
         y += cyChar ;
...
int PASCAL WinMain( HINSTANCE hinst, HINSTANCE hinstPrev, LPSTR lpszCmd, int nCmdShow )
{
   ...
   D( hWDB = wdbOpenSession( NULL, "Font", "wdb.ini", 0 ) ) ;
