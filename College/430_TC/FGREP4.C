...
#define void
    ...
    static enque ();
    static struct trie *deque ();
    static trie *fg_goto ();
    static trie *fg_next ();
    static void fg_search (void);
#endif
     ...
     {    fprintf (stderr, "keyword = %s\n", keyword);
          ...
          {    if (current_p->link_p == NULL) /* if link is not there */
               ...
               {    next_p = current_p->link_p; /* point to linked trie */
                    fprintf (stderr, "->c =%c, c=%c, ->alt_p=%X\n", next_p->c, c, next_p->alt_p);
...
{     struct trie *current_p,
      ...
      {    while (current_p->alt_p != NULL)
           ...
           {    if ((t_p = s_p->link_p) != NULL)
                {    a = t_p->c;
                     enque (t_p);
                     fprintf (stderr, "t_p=%X\n", t_p);
                     temp_p = t_p;
                     while (temp_p->alt_p != NULL)
                     {    fprintf (stderr, "temp_p->alt_p=%X\n",temp_p->alt_p);
                          enque (temp_p->alt_p);
                          temp_p = temp_p->alt_p;
                     }
                     fs_p = s_p->fail_p;
                     while ((fs_p != state_zero_p) && (fg_goto (fs_p, a) == FAIL))
                     {  fprintf (stderr, "fg_goto (fs_p, a)=%X,  ", fg_goto(fs_p,a));
                        fprintf (stderr, "fs_p=%X\n", fs_p);
                        fs_p = fs_p->fail_p;
                     }
                     temp_p->fail_p = fg_next (fs_p, a);
                     if (temp_p->fail_p->end == END_OF_KEY)
                          temp_p->end = END_OF_KEY;
                }
                else
                    if (s_p->alt_p != NULL)
                         enque (s_p->alt_p);
...
{    struct trie *current_p;
     ...
     if (current_p == state_zero_p)
         return (state_zero_p);
     if (current_p == NULL)
         return (FAIL);
     while ((current_p->c != in) && (current_p->alt_p != NULL))
     {   fprintf (stderr, "current_p->c=%c, in=%c\n", current_p->c, in);
         current_p = current_p->alt_p;
     }
     if (current_p->c == in)
         return (current_p);
